...
# Modelos (Baseados no SQL Server original)
...
def cadastro():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.flush() # Para pegar o ID do cadastro
...
def dashboard():
    ...
    denuncias = Denuncia.query.filter_by(fk_usuario_id=user.id).all()
    
    return render_template('dashboard.html', user=user, denuncias=denuncias)
...
def nova_denuncia():
    ...
    if request.method == 'POST':
        ...
        nova_denuncia = Denuncia(endereco=endereco, bairro=bairro, descricao=descricao, fk_usuario_id=user_id)
